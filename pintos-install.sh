...
sudo apt update
sudo apt install build-essential libncurses5 libncurses5-dev libx11-dev xorg-dev qemu -y
...
wget http://lion.sist.shanghaitech.edu.cn/gitlab/IcarusRadio/pintos-anon/-/archive/master/pintos-anon-master.tar.gz
wget https://nchc.dl.sourceforge.net/project/bochs/bochs/2.6.2/bochs-2.6.2.tar.gz


# To extract *.tar.gz files

tar -zxf pintos-anon-master.tar.gz
mv pintos-anon-master pintos
...
cd $SRCDIR
## Idle
sudo rm ./pintos-anon-master.tar.gz
